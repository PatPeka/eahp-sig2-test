...
function renderTopicSlide(slideData, index) {
  ...
  if (slideData.bullets) body.appendChild(renderBulletList(slideData.bullets));
  if (slideData.layout === "use-cases") body.appendChild(renderUseCases(slideData.items));
  if (slideData.layout === "process-flow") body.appendChild(renderProcessFlow(slideData.steps));
  if (slideData.layout === "decisions") body.appendChild(renderDecisions(slideData.items));
  if (slideData.layout === "questions") body.appendChild(renderQuestions(slideData.groups));
  if (slideData.layout === "next-actions") body.appendChild(renderNextActions(slideData.items));
  appendChildren(slide, [renderDeckMeta(index), renderSlideHeader(slideData), body, renderSlideFooter(index)]);
  return slide;
}

function renderDeckMeta(index) {
  const meta = createElement("div", { className: "deck-meta" });
  data.meeting.meta.forEach((item) => meta.appendChild(createElement("span", { text: item })));
...
function renderLogo(logo, className) {
  return createElement("img", { className, attributes: { src: logo.src, alt: logo.alt } });
}

function renderMeetingNotes(index) {
  const wrapper = createElement("label", { className: "meeting-notes" });
  const textarea = createElement("textarea", {
    attributes: { rows: "2", placeholder: "Meeting notes", "aria-label": "Meeting notes" }
  });
  textarea.value = readSavedNotes(index);
  textarea.addEventListener("input", () => saveNotes(index, textarea.value));
  appendChildren(wrapper, [createElement("span", { text: "Meeting notes" }), textarea]);
  return wrapper;
}
...
function renderUseCases(items = []) {
  ...
  items.forEach((item) => {
    ...
    appendChildren(card, [createElement("span", { text: item.id }), createElement("h2", { text: item.title })]);
...
function renderProcessFlow(steps = []) {
  const list = createElement("ol", { className: "process-flow", attributes: { "aria-label": "Meeting process flow" } });
...
function renderDecisions(items = []) {
  ...
  items.forEach((item) => {
    ...
    appendChildren(card, [createElement("span", { text: item.id }), createElement("p", { text: item.text })]);
...
function renderQuestionColumn(group) {
  ...
  appendChildren(column, [createElement("h2", { text: group.title }), renderBulletList(group.items)]);
  return column;
}
...
function previousSlide() {
  if (currentIndex > overviewIndex) showSlide(currentIndex - 1);
}
...
function bindNavigation() {
  ...
  document.addEventListener("keydown", (event) => {
    if (isEditingNotes(event)) return;
